...
const ipchat = require('../chats/ipchat.json');
...
class AgentChat {
    ...
    getIp(){
      ...
      request(optionsIp, (error, response, body) =>{
        if (error){
          console.log("Error requesting Ip")
        }
        const ip = body.ip,
              optionsPost = {
                method: 'POST',
                ...
                body: {
                    event: {
                        '@type': 'line',
                        'text': "<p dir='ltr' style='direction: ltr; text-align: left;'> The IP for the server is " + ip + "</p>",
                        'textType': 'html'
                    }
                }
            };
